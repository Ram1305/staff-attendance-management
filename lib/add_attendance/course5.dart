...
class Course5 extends StatefulWidget{
  @override
  State<StatefulWidget> createState() =>_course5();
}

class _course5 extends State<Course5>{

  final _mlAttendance = FirebaseFirestore.instance.collection('MlAttendance');
  ...
  Future<void> updateAttendance(String studentid, bool present) async {
    try{
      var document = await storage.collection('MlAttendance').doc('$studentid - $convertedDate').get();
      if(document.exists){
        await storage.collection('MlAttendance').doc('$studentid - $convertedDate')
            .update({
          'studentid' : studentid,
          'present' : present,
        ...
        await storage.collection('MlAttendance').doc('$studentid - $convertedDate').set(
            {
              'studentid' : studentid,
              'present' : present,
              'timestamp' : _selectedDate

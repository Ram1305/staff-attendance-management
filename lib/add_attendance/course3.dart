...
class Course3 extends StatefulWidget{
  @override
  State<StatefulWidget> createState() =>_course3();
}

class _course3 extends State<Course3>{

  final _pythonAttendance = FirebaseFirestore.instance.collection('PythonAttendance');
  ...
  Future<void> updateAttendance(String studentid, bool present) async {
    try{
      var document = await storage.collection('PythonAttendance').doc('$studentid - $convertedDate').get();
      if(document.exists){
        await storage.collection('PythonAttendance').doc('$studentid - $convertedDate')
            .update({
          'studentid' : studentid,
          'present' : present,
        ...
        await storage.collection('PythonAttendance').doc('$studentid - $convertedDate').set(
            {
              'studentid' : studentid,
              'present' : present,
              'timestamp' : _selectedDate

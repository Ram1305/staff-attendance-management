...
class Course2 extends StatefulWidget{
  @override
  State<StatefulWidget> createState() =>_course2();
}

class _course2 extends State<Course2>{

  final _cppAttendance = FirebaseFirestore.instance.collection('CppAttendance');
  ...
  Future<void> updateAttendance(String studentid, bool present) async {
    try{
      var document = await storage.collection('CppAttendance').doc('$studentid - $convertedDate').get();
      if(document.exists){
        await storage.collection('CppAttendance').doc('$studentid - $convertedDate')
            .update({
          'studentid' : studentid,
          'present' : present,
        ...
        await storage.collection('CppAttendance').doc('$studentid - $convertedDate').set(
            {
              'studentid' : studentid,
              'present' : present,
              'timestamp' : _selectedDate

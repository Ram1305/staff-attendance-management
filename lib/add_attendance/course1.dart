...
class Course1 extends StatefulWidget{
  @override
  State<StatefulWidget> createState() =>_course1();
}

class _course1 extends State<Course1>{

  final _cAttendance = FirebaseFirestore.instance.collection('CAttendance');
  ...
  Future<void> updateAttendance(String studentid, bool present) async {
    try{
      var document = await storage.collection('CAttendance').doc('$studentid - $convertedDate').get();
      if(document.exists){
        await storage.collection('CAttendance').doc('$studentid - $convertedDate')
            .update({
          'studentid' : studentid,
          'present' : present,
        ...
        await storage.collection('CAttendance').doc('$studentid - $convertedDate').set(
          {
          'studentid' : studentid,
          'present' : present,
          'timestamp' : _selectedDate
                             ...
                             Switch(
                               ...
                               onChanged: (present) {
                                 setState(() {
                                   ...
                                   updateAttendance(data?['studentid'], present);
                                 });
                               },
                             ),
                           ],
